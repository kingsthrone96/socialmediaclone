class Validation {
    ...
    static validate(requestBody, schema, reqKey, schemaKey) {
        if(typeof requestBody[reqKey] !== typeof schema[schemaKey]['type']) {
            ...
            if(typeof requestBody[reqKey] === 'string' && schema[schemaKey][`isAbsolute`] && pattern.test(requestBody[reqKey])) {
        ...
        if(requestBody[reqKey].length < schema[schemaKey]['min'] && requestBody[reqKey].length > 0) {
            Validation.errors.push({
                key: reqKey,
    ...
    compare(requestBody) {
        ...
        reqKeys.forEach(reqKey => {
            schemaKeys.forEach(schemaKey => {
                if(reqKey === schemaKey) {
                    Validation.validate(Validation.requestBody, Validation.schema, reqKey, schemaKey);
                }
            });
        });
    }
